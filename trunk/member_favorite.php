...
<div class="wrapper"> 
  ...
  <section class="main-content"> <span class="top-bg"></span>
      ...
      <section class="grid-holder">
        <section class="grid w-padd">
          ...
          <figure class="column c-one-half">
            ...
            <?
			$count=0;
    $strSQL = " Select * FROM IFAVORITE FAV JOIN IFOODS F ON (FAV.FID=F.FID) JOIN IUSERS U  ON (F.UIDS=U.UIDS) JOIN IFOODTYPE FT ON (F.TYPEID=FT.TYPEID) WHERE FAV.UIDS=".$_SESSION['UIDS'];
	//echo $strSQL;
	...
	?>
          ...
          <? } ?>
          <? if (!$count) {
			  echo "<br><br><br><br>No have favorite food yet.";
		  }?>
          </figure>
        </section>
      </section>
