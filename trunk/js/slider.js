...
$(document).ready(function () {
    ...
    $('.stopClickPropagation').click(function (ev) {
        ev.stopPropagation();
    });
    ...
    if (galleryContainer.length) {
        ...
        var galleryJumpToItem = function (item, speed) {
            ...
            galleryContainerTrack.animate({
                left: galleryContainerCenter - positionLeft - item.width() / 2 + 'px'
            }, speed);
        ...
        $('#featured .gallery .site img').live('mouseenter', function () {
            $(this).ezpz_tooltip({
                offset: 20,
    ...
    $('div.wrapper.showcase').each(function () {
        var currIndex = 0;
        var isAnimated = false;
        $('div.paging a', this).click(function (ev) {
            ev.preventDefault();
            if ($(this).hasClass('active') || isAnimated) {
                return;
            }
            isAnimated = true;
            var currMediaIndex = $($(this).siblings('.active')[0]).index();
            var media = $(this).parent().prev().children();
            $(this).addClass('active').siblings().removeClass('active');
            $(media[currMediaIndex]).removeClass('active');
            $(media[$(this).index()]).addClass('active');
            var ref = this;
            window.setTimeout(function () {
                $(media[currMediaIndex]);
                isAnimated = false;
            }, 750);
        });
        $('div.item', this).each(function (ev) {
            var mediaItems = $('div.media', this).children();
            var ref = this;
            var mediaImages = mediaItems.filter('img').click(function () {
                var index = $(this).index() + 1;
                if (index == mediaItems.length) {
                    index = 0;
                }
                $(ref).find('div.paging a').eq(index).trigger('click');
            });
        });
        var ref = this;
        $('div.right nav ul.list li a', this).click(function (ev) {
            ev.preventDefault();
            var index = $(this).parent().index();
            $('div.items', ref).children().eq(index).addClass('active').siblings().removeClass('active');
            $('div.details', ref).children().eq(index).addClass('active').siblings().removeClass('active');
            $(this).parent().addClass('active').siblings().removeClass('active');
        });
    });
    var jumpToAnchor = function (name) {
        try {
            $('html, body').animate({
                'scrollTop': $('a[name=' + name + ']').offset().top - 90 + 'px'
            }, 1000, 'swing');
        } catch (e) {}
    }
    $('a[href^=#], a[href^="/#"]').click(function (ev) {
    ...
    $('#feedback a.flag').click(function (ev) {
        ev.preventDefault();
        $(this).parent().toggleClass('open');
    });
    $('#newsletterLink').click(function (ev) {
        ev.preventDefault();
        $('#newsletterWrapper').toggleClass('open');
    });
    $('a#loginLink').click(function (ev) {
        ev.preventDefault();
        $('div#loginWrapper').toggleClass('open');
        $('div#lostPasswordWrapper, div#resetPasswordWrapper').removeClass('open');
    });
    $('a#lostPasswordLink').live('mouseup', function (ev) {
        ev.preventDefault();
        $('a#loginLink').click();
        $('div#lostPasswordWrapper').toggleClass('open');
    });
    $('#userPanelLink').click(function (ev) {
        ev.preventDefault();
        $('#userPanelWrapper').toggleClass('open');
    });
});