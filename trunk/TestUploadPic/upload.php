...
*/
$SAVE_PATH = "files/";//$_SERVER['DOCUMENT_ROOT']."/foodcooking/TestUploadPic/MyResize"; //พาสที่ต้องการนำไฟล์ไปเก็บไว้
/**
...
if(isset($_FILES["file"])){
	if($_FILES["file"]["size"]>0){
		...
		//เรียกฟังก์ชั่น Resize
			$hashname = md5_file($_FILES["file"]["tmp_name"]);
			if($newfilename = uploadResizeTo($_FILES["file"], $SAVE_PATH,$hashname, 500, 500)){
				echo 'upload ok <strong>'.$newfilename.'</strong>';
				echo "            ".md5_file($_FILES["file"]["tmp_name"]);
			?>
		
			<img src="<?=$SAVE_PATH.$newfilename;?>" />
			<?
		}else echo 'upload fail';
	}else{
		echo 'Upload Picture Only';
		
		}
		
	if($newfilename = uploadResizeTo($_FILES["file"], $SAVE_PATH,"_".$hashname, 180, 180)){
		echo 'upload pic2 ok <strong>'.$newfilename.'</strong>';
		//echo "_".md5($newfilename);
		?>
		<img src="<?=$SAVE_PATH.$newfilename;?>" />
		<?
	}
	...
	echo 'Please Upload your Picture';
      }
}
?>
